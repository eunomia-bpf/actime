...
are written by the kernel-side engines on the host, outside the sandbox, so it
is not something the agent authored and not something the agent can edit.

## The run record

Real layout, from an actual run (some files exist only when the plane that
writes them ran):
...
~/.local/share/actime/runs/20260804-153012-a3f1/
  ...
  violations.jsonl       one policy match per line, appended live      (policy plane)
  feedback.txt           corrective feedback written by ActPlane       (policy plane)
  policy-audit.jsonl     ActPlane's audit log                          (policy plane)
  ...
  history.log            akeep output, when non-empty                  (history plane)
```
...
The text report prints the header, the plane table, the summary counters, and
the violation table. The Markdown report (`--markdown`, and `report.md` on
disk) adds a **Timeline** section when the run produced violations or events.
The JSON report (`--json`) emits `{manifest, summary, violations, timeline}`.

## `violations.jsonl`

One JSON object per line, appended as matches happen, so you can `tail -f` it
during a long run. The fields are exactly the `Violation` struct Actime reads
back:

```json
{"ts":"2026-08-04T15:32:41Z","rule":"credential-access","effect":"notify","op":"read","target":"/home/dev/.aws/credentials","pid":41180,"comm":"python3","reason":"The agent read credential material. This is reported, not blocked, so that legitimate tooling keeps working. Review it in the run report."}
...
disabled or degraded, the command says so instead of guessing.
